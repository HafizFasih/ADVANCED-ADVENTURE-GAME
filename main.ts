...
console.log(
  chalk.red("\t\t\t\t\t<======="),
  chalk.bgWhite.red.italic.bold("CODE WITH MUHAMMAD FASIH"),
  chalk.red("=======>")
);
console.log(
  chalk.green("\t\t           <---------------"),
  chalk.blue.italic.bold("AN ADVENTURE GAME USING TYPESCRIPT"),
  chalk.green("--------------->\n")
);
console.log(chalk.red.bold("\t\t\t\t\t\t  ( -_•)╦==╤─   - (◣_◢)"));
let healing: number = 7;
let turn: number = 0;
...
console.log("\n");
 class game {
  ...
  userHealth: number = userHP;
  enemyName: string = enemy.name;
  enemyHealth: number = enemyHP;

  punch() {
    let luck: number = Math.floor(Math.random() * 2);
    let randomNum: number = Math.floor(Math.random() * 3);

    let msg: string = "Dummy";
    switch (randomNum) {
      case 0: {
        luck == 0
    ...
    console.log(
      chalk.blue.underline(
        `Remaining health of ${enemy.name}: ${chalk.red(enemyHP)}\n`
      )
    );
...
let mainFunction = async () => {
  ...
  let enemyWins: number = 0;
  for (let i = 1; i <= 3; i++) {
    enemyHP = 100;
    userHP = 100;
    if (i == 1) {
      console.log(
        chalk.magentaBright.underline("\t\t\t<==First Round==>".toUpperCase())
      );
    }
    if (i == 2) {
      console.log(
        chalk.magentaBright.underline("\t\t\t<==Second Round==>".toUpperCase())
      );
    }
    if (i == 3) {
      console.log(
        chalk.magentaBright.underline("\t\t\t<==Third Round==>".toUpperCase())
      );
    }
    ...
    while (condition) {
      ...
      if (fristQuestion === str[0]) {
          ...
          case choose[2]:
            {
              ...
              if (weapon.select == chalk.blueBright("Attack with weapon")) {
                if (purchase == 0) {
                  console.log(
                    chalk.yellow.bold("First you have to purchase the weapon.")
                  );
                }
                if (purchase > 0) {
        ...
        if (healing > 0) {
          healing -= 1;
          console.log(
            chalk.yellow.bold(`Total ${healing} potions are left...`)
          );
          let randomNum: number = Math.floor(Math.random() * 3);
          switch (randomNum) {
            case 0:
          ...
          console.log(chalk.red("\n◝(ᵔᵕᵔ)◜"));
          console.log(
            chalk.magenta.italic.underline("\nNow feeling much better...")
          );
          console.log(
            chalk.blue.underline(
              `${name.userName}'s health status: ${chalk.red(userHP)}\n`
            )
          );
        } else {
          console.log(
            chalk.yellow.bold(
              "Potion is out of stack, can't be able to heal now."
            )
          );
        }
